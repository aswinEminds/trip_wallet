        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center gap-1 px-1 sm:px-3 py-2 transition-all duration-200 flex-1 sm:flex-none border-2 border-transparent ${
                isActive
                  ? "bg-neon-lime border-black shadow-[2px_2px_0px_black] scale-105 sm:scale-110 -translate-y-1"
                  : "hover:bg-gray-100"
              }`}
            >
              <IconComponent 
